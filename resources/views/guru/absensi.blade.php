...
<div class="row">
    <div class="col-md-12 mb-4">
        <div class="card shadow-sm border-0 border-start border-info border-4 rounded-4">
            <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <h4 class="text-info mb-1 fw-bold"><i class="fas fa-calendar-check me-2"></i>Kelola Absensi: {{ $pelajaran->nama_pelajaran }}</h4>
    ...
    <div class="col-md-12">
        <ul class="nav nav-pills guru-attendance-tabs mb-4 bg-white shadow-sm rounded-pill p-2 d-inline-flex overflow-auto text-nowrap" id="pills-tab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link rounded-pill fw-bold px-4 text-dark" id="pills-rekap-tab" data-bs-toggle="pill" data-bs-target="#pills-rekap" type="button" role="tab" aria-controls="pills-rekap" aria-selected="false">
                    <i class="fas fa-list-ol me-1 text-info"></i> Daftar Pertemuan
                </button>
            </li>
        </ul>
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="pills-harian" role="tabpanel" aria-labelledby="pills-harian-tab" tabindex="0">
                <div class="card shadow-sm border-0 bg-white mb-4 rounded-4">
                    <div class="card-body py-3">
                        <form action="{{ route('guru.absensi', $pelajaran->id) }}" method="GET" class="d-flex align-items-center flex-wrap gap-3">
                            <label class="fw-bold text-dark mb-0"><i class="fas fa-calendar-alt me-1 text-info"></i> Pilih Tanggal Absensi:</label>
                            <input type="date" name="tanggal" class="form-control w-auto border-info shadow-sm fw-bold text-info rounded-pill px-3" value="{{ $tanggal }}" onchange="this.form.submit()">

                            @if(isset($riwayat_pertemuan[$tanggal]))
                                <span class="badge bg-info rounded-pill px-3 py-2 shadow-sm">Pertemuan Ke-{{ $riwayat_pertemuan[$tanggal]['pertemuan'] }}</span>
                            @else
                ...
                <div class="card shadow-sm border-0 border-top border-info border-4 rounded-4 overflow-hidden">
                    <div class="card-header bg-white fw-bold text-dark d-flex justify-content-between align-items-center py-3">
                        <span><i class="fas fa-users me-1 text-info"></i> Daftar Siswa Kelas Ini</span>
                        <span class="badge bg-info fs-6 shadow-sm rounded-pill px-3"><i class="far fa-calendar-alt me-1"></i> {{ \Carbon\Carbon::parse($tanggal)->format('d F Y') }}</span>
                    </div>
                    <div class="card-body p-0">
                        <form action="{{ route('guru.absensi.store', $pelajaran->id) }}" method="POST">
                                ...
                                <table class="table table-hover align-middle mb-0 text-nowrap">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="fw-bold text-info">{{ $siswa->name }}</td>
                                            <td class="text-muted">{{ $siswa->nomor_induk ?? '-' }}</td>
                                            <td class="text-center">
                                                <div class="btn-group guru-attendance-status shadow-sm" role="group">
                                                    ...
                                                    <label class="btn btn-outline-info fw-bold px-3" for="sakit_{{ $siswa->id }}">Sakit</label>
                            ...
                            <div class="card-footer bg-light text-end p-3 border-top-0">
                                <button type="submit" class="btn btn-info text-white fw-bold px-4 rounded-pill shadow-sm transition-all">
            ...
            <div class="tab-pane fade" id="pills-rekap" role="tabpanel" aria-labelledby="pills-rekap-tab" tabindex="0">
                <div class="card shadow-sm border-0 border-top border-info border-4 rounded-4 overflow-hidden">
                    <div class="card-header bg-white fw-bold text-dark py-3">
                        <i class="fas fa-list-ol me-1 text-info"></i> Daftar Pertemuan Kelas
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0 text-nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button type="button" class="btn btn-sm btn-outline-info fw-bold rounded-pill px-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalDetail{{ $riwayat['pertemuan'] }}">
