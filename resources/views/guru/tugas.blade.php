    ...
    <div class="col-md-12 mb-4">
        <div class="card shadow-sm border-0 border-start border-info border-4">
            <div class="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <h4 class="text-info mb-1 fw-bold"><i class="fas fa-tasks me-2"></i>Kelola Tugas: {{ $pelajaran->nama_pelajaran }}</h4>
    ...
    <div class="col-md-8 mb-4">
        <div class="card shadow-sm border-0 rounded-4 overflow-hidden border-top border-info border-4">
            <div class="card-header bg-white fw-bold text-dark py-3">
                <i class="fas fa-list me-1 text-info"></i> Daftar Tugas yang Diberikan
            </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if($t->file_tugas)
                                        <a href="{{ asset('uploads/tugas/' . $t->file_tugas) }}" target="_blank" rel="noopener noreferrer" class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-3 py-2 rounded-pill shadow-sm text-decoration-none" title="Buka lampiran tugas">
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-1 flex-wrap">
                                        <a href="{{ route('guru.tugas.jawaban', $t->id) }}" class="btn btn-sm btn-success fw-bold rounded-pill shadow-sm transition-all guru-check-action">
                                            <i class="fas fa-user-check me-1"></i> Cek Jawaban
                                        </a>

                                        <button type="button" class="btn btn-sm btn-outline-info fw-bold rounded-pill shadow-sm transition-all guru-row-action" data-bs-toggle="modal" data-bs-target="#editTugas{{ $t->id }}">
                                            Edit
                                        </button>
                                        ...
                                        <form action="{{ route('guru.tugas.delete', $t->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger fw-bold rounded-pill shadow-sm transition-all guru-row-action" onclick="return confirm('Yakin ingin menghapus tugas ini? Semua data pengumpulan siswa untuk tugas ini juga akan terhapus permanen.')">Hapus</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content rounded-4 border-0 shadow-lg">
                <div class="modal-header bg-info border-0" style="border-top-left-radius: 1rem; border-top-right-radius: 1rem;">
                    <h5 class="modal-title fw-bold text-white"><i class="fas fa-clipboard-check me-2"></i>Detail Tugas: {{ $t->judul_tugas }}</h5>
                ...
                <div class="modal-body p-4 bg-light">
                    ...
                    @if($t->file_tugas)
                        <div class="mt-4 text-center">
                            ...
                            <a href="{{ asset('uploads/tugas/' . $t->file_tugas) }}" target="_blank" rel="noopener noreferrer" class="btn btn-primary fw-bold rounded-pill px-5 py-2 shadow-sm transition-all">
    ...
    <div class="modal fade" id="editTugas{{ $t->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content rounded-4 border-0 shadow">
                <div class="modal-header bg-info border-0" style="border-top-left-radius: 1rem; border-top-right-radius: 1rem;">
                    <h5 class="modal-title fw-bold text-white"><i class="fas fa-edit me-2"></i>Edit Tugas</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
